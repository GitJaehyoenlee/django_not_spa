...
def signup(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            signed_user.send_welcome_email() # FIXME Celary(비동기)로 처리하는 것 추천
...
def profile_edit(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            messages.success(request, "프로필을 수정 했습니다.")
...
def user_follow(request, username):
    ...
    # request.user => follow_user 를 follow 하려고 하는 중임
...
def user_unfollow(request, username):
    ...
    messages.success(request, f"{unfollow_user}님을 언 팔로우 하였습니다.")
